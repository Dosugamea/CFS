...
if(isset($_GET["id"])) {
  $mysql->exec('INSERT INTO `extend_download_queue` (`ID`, `user_id`, `download_id`, `downloaded_version`) VALUES (NULL, "'.$_SESSION['server']["HTTP_USER_ID"].'", "'.$_GET["id"].'", "0"); ');
  echo '<h3>关闭本页后会提示您进行下载。 <a href="javascript:history.go(-1);">返回</a></h3>';
}