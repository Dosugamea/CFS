...
if(isset($_POST['submit'])) {
  if (!is_numeric($_POST['id'])) {
    ...
    $check_uid = $mysql->prepare('SELECT user_id FROM user_login WHERE user_id=?');
    ...
    if ($check_uid->rowCount()) {
      ...
      echo '<h3>注册成功！关闭本窗口即可进入游戏。<br />请牢记您的密码，在官服使用继承码之后需要这个密码。<br /><br />若关闭窗口后仍然无法进入游戏，或者进入游戏时游戏崩溃，请通知开发者！</h3>';
      die();
    }
  }
}
