<?php
function generateRandomLiveFiveKey($note) {
	...
	for($note=0;$note<$max_combo;$note++){
		...
		if($decoded[$note]["effect"]>10){
			...
			if($num>1){
				if($decoded[$last1]["position"]==3)$decoded[$note]["position"]=4;
				else if($decoded[$last1]["position"]==7)$decoded[$note]["position"]=6;
				...
				if($toput[3]+$toput[4]+$toput[6]+$toput[7]+$toput[5]==0){
					for($i=3;$i<=7;$i++){
				...
				if($singlelast>=1){
					$toput[5]=0;
					if($last<=4)$toput[3]=$toput[4]=0;
					if($last>=6)$toput[6]=$toput[7]=0;
				}
				if($toput[3]+$toput[4]+$toput[6]+$toput[7]+$toput[5]==0){
					for($i=3;$i<=7;$i++){
						$toput[$i]=$toput1[$i];
...
function generateRandomLiveOldFiveKey($note) {
	$timing=[];
	foreach($note as $v)
		$timing[]=$v['timing_sec'];
	...
	foreach($note as $k=>&$v) {
		if($v['position']<3)$v['position']+=2;
		else if($v['position']>7)$v['position']-=2;
...
function generateLiveFiveKey($note) {
	$timing=[];
	foreach($note as $v)
		$timing[]=$v['timing_sec'];
	array_multisort($timing,SORT_ASC,$note);
...
function generateRandomLiveLimitlessFiveKey($note) {
	$timing=[];
	foreach($note as $v)
		$timing[]=$v['timing_sec'];
	array_multisort($timing,SORT_ASC,$note);
