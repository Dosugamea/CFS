...
function personalnotice_get() {
	...
	$personal_notice = $mysql->query("SELECT * FROM personalnotice WHERE user_id = ? AND agree = 0", [$uid])->fetch(PDO::FETCH_ASSOC);
	if($personal_notice){
		$ret['has_notice'] = true;
	...
	foreach($global_notice as $i){
		$global_notice_agreement = $mysql->query("SELECT * FROM personalnotice_global WHERE user_id = ? AND notice_id = ?", [$uid, $i['notice_id']])->fetch(PDO::FETCH_ASSOC);
