...
var defaults = { 
	...
	keyboardRadixMin:30,//键盘大小的最小值，默认为30，实际大小为282*99
	keyboardRadixChange:true,//是否允许用户改变键盘大小,该功能仅能完美支持Chrome；当keyboardRadixMin不小于60时，完美支持Safari内核浏览器
	keyfixed:true,//键盘出现位置是否固定在屏幕右下角
...
this.each(function(){ 
	numkeyboardcount++;
	//添加键盘
	if(numkeyboardcount<3){
	$("key").append("<ul id='keyboard_5xbogf8c'>"+
	"<li>1</li>"+
    "<li>2</li>"+
    "<li>3</li>"+
    "<li>4</li>"+
  ...
  "</ul>");	
	...
	if(keyfixed){
		keyboard.css({"position":"fixed","left":"0px","bottom":"0px"});
	}
	exit();
	var shiftnum = 1;
	var shiftbool = false;
	...
	if(keytext==="Clear"||keytext.length<1){
		keyclear();
		evebool = false;
	}
	if(evebool){
	...
	inputele.focus(function(event){	
		...
		if(keyboard.css("display")=="none"){
			if(!keyfixed){
				keyboard.css({"display":"block","left":"30px","top":"430px"});
			}else{
	...
	function mouseDrag() {
		...
		var eleLeft = +eventEle.css("left").split('px')[0];
		var eleTop = +eventEle.css("top").split('px')[0];
		eventEle.mousedown(function(event){
			MAction = true;
			stx = event.pageX;	
			sty = event.pageY;
			eleLeft = +eventEle.css("left").split('px')[0];
			eleTop = +eventEle.css("top").split('px')[0];
			event.preventDefault();
		});
		$("body").mousemove(function(event){			
		if(MAction&&!bMouse){
			...
			eventEle.css({"left":eleLeft+curX,"top":eleTop+curY});
			event.preventDefault();
		}});
		$("body").mouseup(function(event){
			stx = etx = curX = sty = ety = curY = 0;
